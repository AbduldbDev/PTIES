import React from 'react';

interface FormInputProps {
    label: string;
    id: string;
    value: string;
    onChange: (e: React.ChangeEvent<HTMLInputElement>) => void;
    onBlur?: () => void;
    error?: string;
    touched?: boolean;
    type?: string;
    placeholder?: string;
    required?: boolean;
    readOnly?: boolean;
    disabled?: boolean;
    className?: string;
    Icon?: string;
}

export default function FormInput({
    label,
    id,
    value,
    onChange,
    onBlur,
    error,
    touched,
    type = 'text',
    placeholder,
    required = false,
    readOnly = false,
    disabled = false,
    className = '',
    Icon = '',
}: FormInputProps) {
    const inputClass = `w-full rounded-lg border px-3 text-sm lg:text-base py-2 sm:px-4 sm:py-3 transition duration-150 outline-none focus:ring-1 border-primary/20  ${
        required && error && touched ? 'border-red-500 focus:ring-red-500' : 'focus:ring-primary'
    } ${readOnly ? 'bg-gray-100 border-gray-200' : ''} ${disabled ? 'bg-gray-200 cursor-not-allowed' : ''} ${className}`;

    return (
        <div className="relative">
            <label htmlFor={id} className="mb-1 block text-xs font-medium text-gray-700 sm:text-sm">
                {label} {required && <span className="text-red-500">*</span>}
            </label>
            <div className="relative">
                <input
                    type={type}
                    id={id}
                    value={value}
                    onChange={onChange}
                    onBlur={onBlur}
                    className={inputClass}
                    placeholder={placeholder}
                    required={required}
                    readOnly={readOnly}
                    disabled={disabled}
                />
            </div>
            {required && error && touched && <span className="text-xs text-red-500">{error}</span>}
        </div>
    );
}
