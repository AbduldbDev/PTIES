...
type PageProps = {
    ...
    items: PaginatedResponse<Departments>;
};
...
export default function Home() {
    ...
    } = useTableManagement<Departments>({
        data: items.data,
    ...
    const handleDelete = (id: number) => {
        form.delete(`/Admin/structure/department/delete/${id}`);
    };

    const handleView = (id: any) => {
        router.get(`/Admin/structure/department/edit/${id}`);
    };

    const columns: SortableColumn[] = [
        { key: 'parent.title', label: 'Department Head', sortable: true },
        ...
        <>
            ...
            <AppWrapper>
                ...
                <div className="grid grid-cols-1 gap-10 xl:grid-cols-1">
                    <ComponentCard title="All Departments">
                        <TableControls
                                    ...
                                    <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                                        {sortedItems.length > 0 ? (
                                            sortedItems.map((department) => (
                                                <TableRow key={department.id}>
                                                    <TableCell className="text-theme-sm border border-gray-100 px-4 py-3 text-start text-gray-500 dark:border-white/[0.05] dark:text-gray-400">
                                                        {department.parent?.title ?? 'N/A'}
                                                    </TableCell>
                                                    <TableCell className="text-theme-sm border border-gray-100 px-4 py-3 text-start text-gray-500 dark:border-white/[0.05] dark:text-gray-400">
                                                        <i className={`fas ${department.icon} `}></i> {department.title}
                                                    </TableCell>
                                                    <TableCell className="text-theme-sm border border-gray-100 px-4 py-3 text-start text-gray-500 dark:border-white/[0.05] dark:text-gray-400">
                                                        {department.subtitle}
                                                    </TableCell>
                                                    ...
                                                    <TableCell className="text-theme-sm border border-gray-100 px-4 py-3 text-start text-gray-500 capitalize dark:border-white/[0.05] dark:text-gray-400">
                                                        <div className="col-span-1 flex justify-center">
                                                            <div className="flex w-full items-center justify-center gap-2">
                                                                <DeleteConfirm
                                                                    onDeleteConfirmed={() => handleDelete(department.id)}
                                                                    message={`Are you sure you want to delete department named "${department.title}"?`}
                                                                />
                                            ...
                                            <TableRow>
                                                ...
                                                >
                                                    {searchTerm ? `No Department found matching "${searchTerm}"` : 'No Department available'}
                                                </TableCell>
                                            </TableRow>
                                        )}
                                    </TableBody>
