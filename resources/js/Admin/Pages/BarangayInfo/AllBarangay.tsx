...
type PageProps = {
    ...
    items: PaginatedResponse<BarangayInfoProps>;
};
...
export default function Home() {
    ...
    } = useTableManagement<BarangayInfoProps>({
        data: items.data,
    ...
    const handleDelete = (id: number) => {
        form.delete(`/Admin/barangay-info/delete/${id}`);
    };

    const handleView = (id: any) => {
        router.get(`/Admin/barangay-info/edit/${id}`);
    };

    const columns: SortableColumn[] = [
        { key: 'type', label: 'Pakil Side', sortable: true },
                                    ...
                                    <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                                        {sortedItems.length > 0 ? (
                                                    ...
                                                    <TableCell className="border border-gray-100 px-4 py-3 text-start text-theme-sm text-gray-500 capitalize dark:border-white/[0.05] dark:text-gray-400">
                                                        <div className="col-span-1 flex justify-center">
                                                            <div className="flex w-full items-center justify-center gap-2">
                                                                <button
                                                                    aria-label="Edit-btn"
                                                                    onClick={() => handleView(barangayInfo.id)}
                                            ...
                                            <TableRow>
                                                ...
                                                >
                                                    {searchTerm ? `No barangay Info found matching "${searchTerm}"` : 'No barangay Info available'}
                                                </TableCell>
                                            </TableRow>
                                        )}
                                    </TableBody>
