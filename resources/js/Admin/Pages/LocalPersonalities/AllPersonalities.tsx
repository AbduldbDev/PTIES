...
type PageProps = {
    ...
    items: PaginatedResponse<Personality>;
};
...
export default function Home() {
    ...
    } = useTableManagement<Personality>({
        data: items.data,
        initialSort: { key: 'created_at', direction: 'desc' },
        meta: items.meta || undefined,
        customSearchFilter: (item, searchTerm) => {
            const searchLower = searchTerm.toLowerCase();
            return item.category.toLowerCase().includes(searchLower) || item.name.toLowerCase().includes(searchLower);
        },
    });

    const handleDelete = (id: number) => {
        form.delete(`/Admin/local-personalities/delete/${id}`);
    };

    const handleView = (id: any) => {
        router.get(`/Admin/local-personalities/edit/${id}`);
    };
        ...
        <>
            ...
            <AppWrapper>
                ...
                <div className="grid grid-cols-1 gap-10 xl:grid-cols-1">
                    <ComponentCard title="All Local Personalities">
                        <TableControls
                                ...
                                <Table>
                                    ...
                                    <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                                        {sortedItems.length > 0 ? (
                                            sortedItems.map((personalities) => {
                                                const [catIcon, catLabel] = personalities.category
                                                    ? personalities.category.split('|')
                                                    : ['', personalities.category]; // fallback if no "|"

                                                return (
                                                    <TableRow key={personalities.id}>
                                                        <TableCell className="border border-gray-100 px-4 py-3 text-start text-theme-sm text-gray-500 dark:border-white/[0.05] dark:text-gray-400">
                                                            <div className="flex items-center justify-center gap-3">
                                                                <img
                                                                    ...
                                                                    src={
                                                                        personalities.image
                                                                            ? `/storage/${personalities.image}`
                                                                            : '/images/personalities/personalities.png'
                                                                    }
                                                                    alt={personalities.name}
                                                                />
                                                            </div>
                                                        </TableCell>
                                                        ...
                                                        <TableCell className="border border-gray-100 px-4 py-3 text-start text-theme-sm text-gray-500 dark:border-white/[0.05] dark:text-gray-400">
                                                            <div className="flex items-center gap-2">
                                                                {catIcon && <i className={`fas ${catIcon} `}></i>}
                                                                <span>{catLabel}</span>
                                                            </div>
                                                        </TableCell>
                                                        ...
                                                        <TableCell className="w-[600px] border border-gray-100 px-4 py-3 text-start text-theme-sm text-gray-500 capitalize dark:border-white/[0.05] dark:text-gray-400">
                                                            {personalities.description.length > 200
                                                                ? personalities.description.substring(0, 200) + '...'
                                                        ...
                                                        <TableCell className="border border-gray-100 px-4 py-3 text-start text-theme-sm text-gray-500 capitalize dark:border-white/[0.05] dark:text-gray-400">
                                                            <div className="col-span-1 flex justify-center">
                                                                <div className="flex w-full items-center justify-center gap-2">
                                                                    <DeleteConfirm
                                                                        onDeleteConfirmed={() => handleDelete(personalities.id)}
                                                                        message={`Are you sure you want to delete Local personality named ${personalities.name}?`}
                                                                    />
                                                                    <button
                                                                        aria-label="Edit-btn"
                                                                        onClick={() => handleView(personalities.id)}
                                            ...
                                            <TableRow>
                                                ...
                                                >
                                                    {searchTerm
                                                        ? `No Local Personalities found matching "${searchTerm}"`
                                                        : 'No Local Personalities available'}
                                                </TableCell>
                                            </TableRow>
                                        )}
                                    </TableBody>
