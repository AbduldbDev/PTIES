...
type PageProps = {
    ...
    items: PaginatedResponse<EstablishmentProps>;
};
...
export default function Home() {
    ...
    } = useTableManagement<EstablishmentProps>({
        data: items.data,
    ...
    const handleDelete = (id: number) => {
        form.delete(`/Admin/establishment/delete/${id}`);
    };

    const handleView = (id: any) => {
        router.get(`/Admin/establishment/edit/${id}`);
    };

    const columns: SortableColumn[] = [
        { key: 'type', label: 'Establishment', sortable: true },
                                                    ...
                                                    <TableCell className="text-theme-sm border border-gray-100 px-4 py-3 text-start text-gray-500 capitalize dark:border-white/[0.05] dark:text-gray-400">
                                                        <div className="col-span-1 flex justify-center">
                                                            <div className="flex w-full items-center justify-center gap-2">
                                                                <DeleteConfirm
                                                                    onDeleteConfirmed={() => handleDelete(establishment.id)}
                                                                    message={`Are you sure you want to delete tour guide named ${establishment.name}?`}
                                                                />
