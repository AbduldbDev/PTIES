...
type PageProps = {
    ...
    items: PaginatedResponse<Products>;
};
...
export default function Home() {
    ...
    const handleDelete = (id: number) => {
        form.delete(`/Admin/local-products/delete/${id}`);
    };

    const handleView = (id: any) => {
        router.get(`/Admin/local-products/edit/${id}`);
    };
        ...
        <>
            ...
            <AppWrapper>
                ...
                <div className="grid grid-cols-1 gap-10 xl:grid-cols-1">
                    <ComponentCard title="All Local products">
                        <TableControls
                                    ...
                                    <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                                        {sortedItems.length > 0 ? (
                                            sortedItems.map((products) => {
                                                const [catIcon, catLabel] = products.category
                                                    ? products.category.split('|')
                                                    : ['', products.category]; // fallback if no "|"
                                                        ...
                                                        <TableCell className="border border-gray-100 px-4 py-3 text-start text-theme-sm text-gray-500 capitalize dark:border-white/[0.05] dark:text-gray-400">
                                                            <div className="col-span-1 flex justify-center">
                                                                <div className="flex w-full items-center justify-center gap-2">
                                                                    <DeleteConfirm
                                                                        onDeleteConfirmed={() => handleDelete(products.id)}
                                                                        message={`Are you sure you want to delete Local Products named ${products.name}?`}
                                                                    />
                                                                    <button
                                                                        aria-label="Edit-btn"
                                                                        onClick={() => handleView(products.id)}
                                            ...
                                            <TableRow>
                                                ...
                                                >
                                                    {searchTerm ? `No Local products found matching "${searchTerm}"` : 'No Local products available'}
                                                </TableCell>
                                            </TableRow>
                                        )}
                                    </TableBody>
