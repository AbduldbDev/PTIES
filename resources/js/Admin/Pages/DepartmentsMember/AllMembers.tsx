...
type PageProps = {
    ...
    items: PaginatedResponse<DepartmentMember>;
};
...
export default function Home() {
    ...
    } = useTableManagement<DepartmentMember>({
        data: items.data,
    ...
    const handleDelete = (id: number) => {
        form.delete(`/Admin/structure/members/delete/${id}`);
    };

    const handleView = (id: any) => {
        router.get(`/Admin/structure/members/edit/${id}`);
    };

    const columns: SortableColumn[] = [
        { key: 'is_leader', label: 'Type', sortable: true },
        { key: 'department.title', label: 'Department', sortable: true },
    ...
    return (
        <>
            ...
            <AppWrapper>
                ...
                <PageBreadcrumb pageTitle="Department Members Management" />

                <div className="grid grid-cols-1 gap-10 xl:grid-cols-1">
                    <ComponentCard title="All Department Members">
                        <TableControls
                                                ...
                                                <TableRow key={member.id}>
                                                    <TableCell className="text-theme-sm border border-gray-100 px-4 py-3 text-center text-gray-500 capitalize dark:border-white/[0.05] dark:text-gray-400">
                                                        {member.is_leader == false ? (
                                                            <span className="rounded-full bg-blue-800/50 px-3 py-1 whitespace-nowrap text-black dark:text-gray-300">
                                                                Member
                                                            </span>
                                                        ) : (
                                                            <span className="rounded-full bg-green-800/50 px-3 py-1 whitespace-nowrap text-black dark:text-gray-300">
                                                                Head
                                                            </span>
                                                        )}
                                                    </TableCell>
                                                    ...
                                                    <TableCell className="text-theme-sm border border-gray-100 px-4 py-3 text-start text-gray-500 capitalize dark:border-white/[0.05] dark:text-gray-400">
                                                        <div className="col-span-1 flex justify-center">
                                                            <div className="flex w-full items-center justify-center gap-2">
                                                                <DeleteConfirm
                                                                    onDeleteConfirmed={() => handleDelete(member.id)}
                                                                    message={`Are you sure you want to delete member named "${member.name}"?`}
                                                                />
