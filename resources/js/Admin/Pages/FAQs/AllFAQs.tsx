...
type PageProps = {
    ...
    items: PaginatedResponse<FAQsProps>;
};
...
export default function Home() {
    ...
    } = useTableManagement<FAQsProps>({
        data: items.data,
        initialSort: { key: 'created_at', direction: 'desc' },
        meta: items.meta || undefined,
        customSearchFilter: (item, searchTerm) => {
            const searchLower = searchTerm.toLowerCase();
            return item.question.toLowerCase().includes(searchLower) || item.answer.toLowerCase().includes(searchLower);
        },
    });

    const handleDelete = (id: number) => {
        form.delete(`/Admin/faqs/delete/${id}`);
    };

    const handleView = (id: any) => {
        router.get(`/Admin/faqs/edit/${id}`);
    };
                                                    ...
                                                    <TableCell className="text-theme-sm border border-gray-100 px-4 py-3 text-start text-gray-500 capitalize dark:border-white/[0.05] dark:text-gray-400">
                                                        <div className="col-span-1 flex justify-center">
                                                            <div className="flex w-full items-center justify-center gap-2">
                                                                <DeleteConfirm
                                                                    onDeleteConfirmed={() => handleDelete(faqs.id)}
                                                                    message={`Are you sure you want to delete this FAQs?`}
                                                                />

                                                                <button
                                                                    aria-label="Edit-btn"
                                                                    onClick={() => handleView(faqs.id)}
