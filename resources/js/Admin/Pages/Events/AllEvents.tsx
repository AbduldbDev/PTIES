...
type PageProps = {
    ...
    items: PaginatedResponse<eventsProps>;
};
...
export default function Home() {
    ...
    } = useTableManagement<eventsProps>({
        data: items.data,
    ...
    const handleDelete = (id: number) => {
        form.delete(`/Admin/events/delete/${id}`);
    };
    ...
    const columns: SortableColumn[] = [
        ...
        { key: 'start_date', label: 'Date', sortable: true, align: 'center' },
                                                ...
                                                <TableRow key={events.id}>
                                                    ...
                                                    <TableCell className="border border-gray-100 px-4 py-3 text-start text-theme-sm text-gray-500 dark:border-white/[0.05] dark:text-gray-400">
                                                        {events.description.length > 200
                                                            ? events.description.substring(0, 200) + '...'
                                                            : events.description}
                                                    </TableCell>
                                                    <TableCell className="border border-gray-100 px-4 py-3 text-start text-theme-sm text-gray-500 dark:border-white/[0.05] dark:text-gray-400">
                                                        {events.start_date === events.end_date
                                                            ...
                                                            : `${new Date(events.start_date).toLocaleDateString('en-US', {
                                                                  ...
                                                                  month: 'long',
                                                                  day: 'numeric',
                                                              })} - ${new Date(events.end_date).toLocaleDateString('en-US', {
                                                                  year: 'numeric',
                                                                  month: 'long',
                                                                  day: 'numeric',
                                                              })}`}
                                                    ...
                                                    <TableCell className="border border-gray-100 px-4 py-3 text-start text-theme-sm text-gray-500 capitalize dark:border-white/[0.05] dark:text-gray-400">
                                                        <div className="col-span-1 flex justify-center">
                                                            <div className="flex w-full items-center justify-center gap-2">
                                                                <DeleteConfirm
                                                                    onDeleteConfirmed={() => handleDelete(events.id)}
                                                                    message={`Are you sure you want to delete this events?`}
                                                                />

                                                                <button
                                                                    aria-label="Edit-btn"
                                                                    onClick={() => handleView(events.id)}
